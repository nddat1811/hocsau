...
def select_relief(x: np.ndarray, y: np.ndarray, num_features: int) -> np.ndarray:
    scores = relief_f_scores(x, y)
    return np.argsort(scores)[::-1][:num_features]


def select_sfs(x: np.ndarray, y: np.ndarray, num_features: int) -> np.ndarray:
    selector = SequentialFeatureSelector(
        KNeighborsRegressor(n_neighbors=5),
        n_features_to_select=min(num_features, x.shape[1]),
        direction="forward",
